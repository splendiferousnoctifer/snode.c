...
FileReader::FileReader(int fd, io::pipe::Sink& sink)
    : Descriptor(fd) {
    ReadEventReceiver::enable(fd);
    Source::connect(sink);
}

FileReader* FileReader::connect(const std::string& path, io::pipe::Sink& writeStream, const std::function<void(int err)>& onError) {
    FileReader* fileReader = nullptr;

    int fd = io::system::open(path.c_str(), O_RDONLY);
...
void FileReader::readEvent() {
    ...
    ssize_t ret = io::system::read(getFd(), junk, MFREADSIZE);
