...
#include "io/timer/SingleshotTimer.h"
...
namespace io::timer {
    ...
    Timer::singleshotTimer(const std::function<void(const void*)>& dispatcher, const struct timeval& timeout, const void* arg) {
        ...
        core::EventLoop::instance().getTimerEventDispatcher().add(st);
    ...
    IntervalTimer& Timer::intervalTimer(const std::function<void(const void*, const std::function<void()>& stop)>& dispatcher,
        ...
        core::EventLoop::instance().getTimerEventDispatcher().add(ct);
    ...
    void Timer::cancel() {
        core::EventLoop::instance().getTimerEventDispatcher().remove(this);
    }
